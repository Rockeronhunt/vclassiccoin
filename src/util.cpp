...
#else
    const char* pszModule = "VCLASSICCOIN";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VCLASSICCOIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VCLASSICCOIN
    // Mac: ~/Library/Application Support/VCLASSICCOIN
    // Unix: ~/.VCLASSICCOIN
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VCLASSICCOIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "VCLASSICCOIN.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "VCLASSICCOINd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong VCLASSICCOIN will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("VCLASSICCOIN"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
