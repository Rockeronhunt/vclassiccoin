...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("VCLASSICCOIN-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #VCLASSICCOINTEST2\r");
            Send(hSocket, "WHO #VCLASSICCOINTEST2\r");
        } else {
            // randomly join #VCLASSICCOIN00-#VCLASSICCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #VCLASSICCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #VCLASSICCOIN%02d\r", channel_number).c_str());
        }
